// ── Colorize module ported from pixel-agents ──────────────────

import type { ColorValue, SpriteData } from './types';
...
function colorizeSprite(sprite: SpriteData, color: ColorValue): SpriteData {
  ...
  for (const row of sprite) {
    ...
    for (const pixel of row) {
      ...
      const alpha = pixel.length > 7 ? parseInt(pixel.slice(7, 9), 16) : 255;
      const hex = hslToHex(h, s / 100, lightness);
      newRow.push(alpha < 255 ? `${hex}${alpha.toString(16).padStart(2, '0').toUpperCase()}` : hex);
    }
    result.push(newRow);
  }
  return result;
...
function hslToHex(h: number, s: number, l: number): string {
  ...
  return `#${clamp((r1 + m) * 255).toString(16).padStart(2, '0')}${clamp((g1 + m) * 255).toString(16).padStart(2, '0')}${clamp((b1 + m) * 255).toString(16).padStart(2, '0')}`.toUpperCase();
}
